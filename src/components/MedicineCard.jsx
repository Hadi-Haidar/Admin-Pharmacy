...
const MedicineCard = ({ medicine, onEdit, onDelete, onImageClick }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
      ...
      <div 
        className="relative h-64 bg-white cursor-pointer group"
        onClick={() => onImageClick && onImageClick(medicine)}
      >
        {medicine.frontImageUrl ? (
          <>
            <img
              ...
              className="w-full h-full object-contain p-4 transition-transform duration-300 group-hover:scale-105 relative z-10"
            />
            <div className="absolute inset-0 bg-white bg-opacity-0 group-hover:bg-opacity-80 transition-all duration-300 flex items-center justify-center pointer-events-none">
              <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-emerald-600 rounded-full p-3 shadow-lg">
        ...
        <div className="absolute top-3 right-3 z-20">
          <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold shadow-lg ${getStatusColor(medicine.status)}`}>
            <span className={`w-2 h-2 ${getStatusDot(medicine.status)} rounded-full mr-2 animate-pulse`}></span>
